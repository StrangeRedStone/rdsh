...
**暂未附许可证**（2026-09-19 的决定）⇒ 默认**保留全部权利**，他人不得合法使用、修改或再分发。

> 若要改为 MIT，只需两步：在仓库根添加 `LICENSE`（MIT 全文），并把本节换成一行声明。脚本本身无第三方代码依赖；若将来把 `dsh-patches/`（补丁派生于 MIT 的上游 DSH）一并发布，需随附上游版权与许可声明。
